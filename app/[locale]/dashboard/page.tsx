      ...
      <div className="max-w-7xl mx-auto space-y-8">
        {countsError && (
          <Alert variant="destructive" className="border-destructive/50">
            ...
            <AlertTitle>{t('couldNotLoadOverview')}</AlertTitle>
        ...
        {chartsError && (
          <Alert variant="destructive" className="border-destructive/50">
            <AlertCircle className="h-4 w-4" />
            <AlertTitle>{t('couldNotLoadCharts')}</AlertTitle>
            <AlertDescription>{chartsError}</AlertDescription>
          </Alert>
        )}
        <div>
          <h2 className="text-4xl font-bold text-foreground tracking-wider">{t('overview')}</h2>
          <p className="text-muted-foreground mt-2 uppercase text-xs tracking-widest">{t('welcomeBack')}</p>
        </div>
              ...
              <Card key={index} className="border border-border/50 hover:border-primary/30 hover:shadow-lg transition-all duration-300">
                ...
                <CardContent>
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-emerald-400 font-semibold uppercase tracking-wider">+{kpi.change}% {t('mom')}</p>
                  </div>
                </CardContent>
              </Card>
            )
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card className="border border-border/50 hover:border-primary/30 hover:shadow-lg transition-all duration-300">
            <CardHeader>
              <CardTitle className="text-sm uppercase tracking-wider text-primary">{t('totalMembers')}</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-3xl font-bold text-foreground font-mono">{counts.memberCount}</p>
              <p className="text-xs text-muted-foreground mt-2">
                {counts.membersThisMonth > 0 ? t('newMembersThisMonth', { count: counts.membersThisMonth }) : t('noNewMembersThisMonth')}
              </p>
            </CardContent>
          </Card>
          <Card className="border border-border/50 hover:border-primary/30 hover:shadow-lg transition-all duration-300">
            <CardHeader>
              <CardTitle className="text-sm uppercase tracking-wider text-primary">{t('activeTrainers')}</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-3xl font-bold text-foreground font-mono">{counts.trainerCount}</p>
              <p className="text-xs text-muted-foreground mt-2">{t('acrossAllBranches')}</p>
            </CardContent>
          </Card>
          <Card className="border border-border/50 hover:border-primary/30 hover:shadow-lg transition-all duration-300">
            <CardHeader>
              <CardTitle className="text-sm uppercase tracking-wider text-primary">{t('branches')}</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-3xl font-bold text-foreground font-mono">{counts.branchCount}</p>
              <p className="text-xs text-muted-foreground mt-2">{t('operationalLocations')}</p>
            </CardContent>
          </Card>
        </div>

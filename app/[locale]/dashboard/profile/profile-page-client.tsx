...
import { Mail, Phone, MapPin, Building, Shield, CheckCircle, CreditCard, Calendar, Users, Edit2, Save, X } from 'lucide-react'
import { updateUserProfile } from '@/app/actions/profile'
import { updatePassword } from '@/app/actions/auth'
import { toast } from 'sonner'
import type { ProfileUser } from '@/app/actions/profile'
import type { SubscriptionInfo } from '@/lib/types/platform-plans'
...
export function ProfilePageClient({ userProfile, subscription }: ProfilePageClientProps) {
    ...
    <main className="p-6 lg:p-8">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-2 uppercase text-xs tracking-widest">{t('personalInfo')}</p>
        </div>
                ...
                {isEditing ? (
                  <div className="space-y-3">
                    <div className="space-y-2">
                      <Label className="text-xs text-muted-foreground uppercase tracking-wider">{tCommon('name')}</Label>
                      <Input
                        value={editName}
          ...
          <CardContent className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-4">
                <h3 className="font-semibold uppercase text-sm tracking-wider text-primary border-b border-primary/30 pb-2">{t('personalInfo')}</h3>
                <div className="space-y-4">
                  <div className="flex items-start gap-4">
                    <Mail className="w-5 h-5 text-primary mt-1" />
                    <div>
                      <p className="text-xs text-muted-foreground uppercase tracking-wider">{tCommon('email')}</p>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <p className="text-xs text-muted-foreground uppercase tracking-wider">{tCommon('phone')}</p>
                      {isEditing ? (
                        <Input
                          value={editPhone}
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <p className="text-xs text-muted-foreground uppercase tracking-wider">{t('contactInfo')}</p>
                      {isEditing ? (
                        <Input
                          value={editLocation}
              ...
              {/* Subscription Info */}
              <div className="space-y-4">
                <h3 className="font-semibold uppercase text-sm tracking-wider text-primary border-b border-primary/30 pb-2">{tCommon('status')}</h3>
                <div className="space-y-4">
                  <div className="flex items-start gap-4">
                    <Building className="w-5 h-5 text-success mt-1" />
                    <div>
                      <p className="text-xs text-muted-foreground uppercase tracking-wider">{tCommon('status')}</p>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <p className="text-xs text-muted-foreground uppercase tracking-wider">{tCommon('active')}</p>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <p className="text-xs text-muted-foreground uppercase tracking-wider">{tCommon('status')}</p>
                      <p className="text-sm font-mono text-success mt-1">{tCommon('active').toUpperCase()}</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <Card className="card-cyber">
          <CardHeader>
            <CardTitle className="flex items-center gap-3 text-xl">
              ...
              <span>{tSettings('security')}</span>
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="space-y-3">
              <h4 className="font-medium text-sm uppercase tracking-wider text-muted-foreground">{tSettings('changePassword')}</h4>
              <div className="space-y-2">
                <Label htmlFor="newPassword" className="text-xs uppercase tracking-wider">{tSettings('newPassword')}</Label>
                <Input
                  id="newPassword"
                  type="password"
              ...
              <div className="space-y-2">
                <Label htmlFor="confirmPassword" className="text-xs uppercase tracking-wider">{tSettings('confirmNewPassword')}</Label>
                <Input
                  id="confirmPassword"
                  type="password"
